...
export const updateProfile=(name,email,address,city,country,pinCode)=>async(dispatch)=>{
 ...
 export const placeOrder=(orderItems,shippingInfo,paymentMethod,itemsPrice,taxPrice,shippingCharges,totalAmount,paymentInfo)=>async(dispatch)=>{
   try {
      ...
      //  console.log({orderItems,shippingInfo,paymentMethod,itemsPrice,taxPrice,shippingCharges,totalAmount,paymentInfo})
            const {data}=await axios.post(`${server}/order/new`,{
                orderItems,shippingInfo,paymentMethod,itemsPrice,taxPrice,shippingCharges,totalAmount,paymentInfo
            },{
                headers:{
                    "Content-Type":"application/json"
                },
...
export const updateProduct=(id,name,description,price,stock,category)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.put(`${server}/product/single/${id}`,{
...
export const updateProductImage=(productId,formData)=>async(dispatch)=>{
    try {
       ...
       const {data}=await axios.post(`${server}/product/images/${productId}`,
       formData,{
        headers:{
            "Content-Type": "multipart/form-data",
        },
