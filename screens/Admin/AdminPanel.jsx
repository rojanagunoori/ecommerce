...
//console.log("products",products)
  ...
  return (
    ...
    <View style={defaultStyle}>
        ...
        {loading?(<Loader/>):(
            <>
            <View style={{ color: 'white',backgroundColor:colors.color3,borderRadius:20,alignItems:"center"}} >
  ...
  <ProductListHeading/>
           <ScrollView showsVerticalScrollIndicator={false}>
            <View>
                {
                //!loadingDelete.loading===false &&
                    products.map((item,index)=>(
                       <ProductListItem  key={item._id} navigate={navigation}
                       deleteProductHandler={deleteProductHandler}
                     
  id={item._id} i={index} price={item.price} 
                       stock={item.stock} name={item.name} 
                       category={item?.category?.category} imgSrc={item.images[0]?.url} />
